...
test("protects, authenticates, shows the investment graph, and opens an investment", async ({ page }) => {
  ...
  await expect(companyAllocationRows.locator(".lp-figure-bar-label > a > .lp-link-arrow")).toHaveCount(44);
  ...
  await expect(finalizingAllocationRow).toContainText("Will go to");
  await expect(finalizingAllocationRow).not.toContainText("H256");
  await expect(finalizingAllocationRow).not.toContainText("of vehicle");
  ...
  await expect(finalizingAllocationRow.locator(".lp-link-arrow")).toHaveCount(3);
  await expect(finalizingAllocationRow).toContainText("$112,000");
  await expect(finalizingAllocationRow).toContainText("16.9%");
  const andurilAllocationRow = companyAllocationRows.nth(1);
  await expect(andurilAllocationRow).toContainText("Anduril");
  await expect(andurilAllocationRow).toContainText("$88,000");
  await expect(andurilAllocationRow).toContainText("13.3%");
  await expect(andurilAllocationRow.locator("small")).toHaveCount(0);
  ...
  const performancePositionLinks = performanceRows.locator(".lp-performance-position-link");
  await expect(performancePositionLinks).toHaveCount(44);
  await expect(performancePositionLinks.locator(".lp-link-arrow")).toHaveCount(44);
  ...
  const h256AndurilLink = h256AndurilMark.getByRole("link", { name: "Anduril", exact: true });
  await expect(h256AndurilLink).toHaveAttribute("href", "/lp/investments/09-h256-series-3");
  await h256AndurilLink.hover();
  await expect(h256AndurilLink).toHaveCSS("text-decoration-line", "none");
  await expect(h256AndurilLink.locator(".lp-link-arrow")).toHaveText("↗");
  ...
  const h256MarkSourceLink = h256AndurilMark.getByRole("link", { name: "$100B assumed valuation", exact: true });
  await expect(h256MarkSourceLink.locator(".lp-link-arrow")).toHaveText("↗");
  await expect(h256MarkSourceLink).toHaveCSS("text-decoration-line", "none");
